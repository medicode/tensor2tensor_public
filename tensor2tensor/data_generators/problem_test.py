  ...
  def testMaybeChunk(self):
    class MockedDatasetProblem(algorithmic.TinyAlgo):
      ...
      def dataset(self,
                  ...
                  max_records=-1,
                  only_last=False):
        ex1_inputs = tf.convert_to_tensor([0, 1], dtype=np.int32)
        ex1_targets = tf.convert_to_tensor([2, 3, 5], dtype=np.int32)
        example = {'inputs': ex1_inputs, 'targets': ex1_targets}
        types = {'inputs': [tf.int32], 'targets': [tf.int32]}
        shapes = {'inputs': (1, 2,), 'targets': (3,)}
    ...
    for example in dataset.take(1):
      ...
      example['inputs'] = debug_tfprint(message='hello', tvar=example['inputs'])
      asserts.append(tf.assert_equal(tf.math.floormod(tf.shape(example['inputs'])[1], hparams.chunk_length), 0))

    with tf.Session() as sess:
      with tf.control_dependencies(asserts):
        sess.run(example['inputs'])
