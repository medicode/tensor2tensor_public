...
def padded_cross_entropy(logits,
  ...
  with tf.name_scope("padded_cross_entropy", values=[logits, labels]):
    ...
    print_op = tf.print('is_nam(logits) before cast', logits.dtype, tf.debugging.is_nan(logits))
    with tf.control_dependencies([print_op]):
      logits = tf.identity(logits)
    logits = tf.cast(logits, tf.float32)
    print_op = tf.print('is_nan(logits) after cast', logits.dtype, tf.debugging.is_nan(logits))
    with tf.control_dependencies([print_op]):
