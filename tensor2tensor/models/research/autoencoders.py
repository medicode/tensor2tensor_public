...
class AutoencoderBasic(t2t_model.T2TModel):
  ...
  def image_summary(self, name, image_logits, max_outputs=1):
    ...
    if len(image_logits.get_shape()) != 5:
      tf.compat.v1.logging.info("Not generating image summary, maybe not an image.")
      return
  ...
  def bottleneck(self, x):
    with tf.compat.v1.variable_scope("bottleneck"):
      ...
      x = tf.compat.v1.layers.dense(x, hparams.bottleneck_bits, name="bottleneck")
      if hparams.mode == tf_estimator.ModeKeys.TRAIN:
        noise = 2.0 * tf.random.uniform(common_layers.shape_list(x)) - 1.0
        return tf.tanh(x) + noise * hparams.bottleneck_noise, 0.0
      return tf.tanh(x), 0.0

  def unbottleneck(self, x, res_size, reuse=None):
    with tf.compat.v1.variable_scope("unbottleneck", reuse=reuse):
      x = tf.compat.v1.layers.dense(x, res_size, name="dense")
      return x
  ...
  def body(self, features):
    ...
    if (hparams.mode != tf_estimator.ModeKeys.PREDICT
      ...
      if not is_training:
        ...
        if common_layers.should_generate_summaries():
          tf.compat.v1.summary.scalar("nomix_p_bottleneck", nomix_p)
        rand = tf.random.uniform(common_layers.shape_list(x))
        ...
        xb_loss = tf.reduce_mean(tf.reduce_sum(
            tf.math.squared_difference(x_stop, b), axis=-1))
        ...
        x = tf.compat.v1.where(tf.less(rand, nomix_p), b, x)
    ...
    if hparams.mode == tf_estimator.ModeKeys.PREDICT:
      if hparams.use_vq_loss:
        ...
        reconstr = tf.compat.v1.layers.dense(res, vocab_size, name="autoencoder_final")
    ...
    if hparams.use_vq_loss:
      ...
      if hparams.mode != tf_estimator.ModeKeys.TRAIN:
        vq_temperature = None
      with tf.compat.v1.variable_scope("vq_loss"):
    ...
    if hparams.gan_loss_factor != 0.0:
      ...
      if hparams.use_vq_loss:
        with tf.compat.v1.variable_scope("vq_loss", reuse=True):
          update_means = tf.less(tf.random.uniform([]), update_means_factor)
        ...
        reconstr_gan = tf.compat.v1.layers.dense(
  ...
  def sample(self, features=None, shape=None):
    ...
    return 2.0 * tf.random.uniform(size) - 1.0
...
class AutoencoderAutoregressive(AutoencoderBasic):
  ...
  def body(self, features):
    ...
    if hparams.autoregressive_mode == "conv3":
      ...
      res = tf.compat.v1.layers.dense(res, vocab_size, name="autoregressive_final")
...
class AutoencoderResidual(AutoencoderAutoregressive):
  ...
  def encoder(self, x):
    with tf.compat.v1.variable_scope("encoder"):
      ...
      residual_conv = tf.compat.v1.layers.conv2d
      if hparams.residual_use_separable_conv:
        residual_conv = tf.compat.v1.layers.separable_conv2d
      # Down-convolutions.
      for i in range(hparams.num_hidden_layers):
        with tf.compat.v1.variable_scope("layer_%d" % i):
          ...
          x = tf.compat.v1.layers.conv2d(
              ...
              name="strided")
          y = x
          y = tf.nn.dropout(y, rate=1 - (1.0 - hparams.residual_dropout))
    ...
    with tf.compat.v1.variable_scope("decoder"):
      ...
      for i in range(hparams.num_hidden_layers):
        ...
        if is_training:
          ...
          if common_layers.should_generate_summaries():
            tf.compat.v1.summary.scalar("nomix_p_%d" % j, nomix_p)
        ...
        with tf.compat.v1.variable_scope("layer_%d" % i):
          ...
          for r in range(hparams.num_residual_layers):
            ...
            y = residual_conv(
                ...
                activation=common_layers.belu,
                name="residual_%d" % r)
          x += tf.nn.dropout(y, rate=1 - (1.0 - hparams.residual_dropout))
          x = common_layers.layer_norm(x, name="ln")
          x = common_attention.add_timing_signal_nd(x)
          if encoder_layers is not None:
            ...
            if is_training:  # Mix at the beginning of training.
              rand = tf.random.uniform(common_layers.shape_list(x_mix))
              x_mix = tf.compat.v1.where(tf.less(rand, nomix_p), x_mix, enc_x)
...
class AutoencoderResidualVAE(AutoencoderResidual):
  ...
  def bottleneck(self, x):
    ...
    with tf.compat.v1.variable_scope("vae"):
      ...
      epsilon = tf.random.normal(x_shape[:-1] + [z_size])
...
class AutoencoderBasicDiscrete(AutoencoderAutoregressive):
  ...
  def bottleneck(self, x):
    hparams = self.hparams
    x = tf.tanh(tf.compat.v1.layers.dense(x, hparams.bottleneck_bits, name="bottleneck"))
  ...
  def sample(self, features=None, shape=None):
    ...
    size = size if shape is None else shape
    rand = tf.random.uniform(size)
    return 2.0 * tf.cast(tf.less(0.5, rand), dtype=tf.float32) - 1.0
...
class AutoencoderResidualDiscrete(AutoencoderResidual):
  ...
  def variance_loss(self, b):
    part = tf.random.uniform(common_layers.shape_list(b))
    selection = tf.cast(tf.less(part, tf.random.uniform([])), dtype=tf.float32)
  ...
  def sample(self, features=None, shape=None):
    ...
    size = size if shape is None else shape
    rand = tf.random.uniform(size)
    res = 2.0 * tf.cast(tf.less(0.5, rand), dtype=tf.float32) - 1.0
...
class AutoencoderOrderedDiscrete(AutoencoderResidualDiscrete):
  ...
  def bottleneck(self, x):  # pylint: disable=arguments-differ
    ...
    if hparams.mode == tf_estimator.ModeKeys.TRAIN:
      ...
      log_p = tf.math.log1p(-float(noise) / 2) / float(hparams.bottleneck_bits)
      ...
      ordered_noise = tf.random.uniform(tf.shape(x))
      # We want our noise to be 1s at the start and random {-1, 1} bits later.
      ordered_noise = tf.cast(tf.less(noise_mask, ordered_noise), dtype=tf.float32)
...
class AutoencoderDualDiscrete(AutoencoderResidualDiscrete):
  ...
  def bottleneck(self, x):
    ...
    rand = tf.random.uniform(common_layers.shape_list(bt))
    br = tf.compat.v1.where(rand < 0.5, bt, bi)  # Break ties at random.
    bs = tf.compat.v1.where(shared == 0, br, shared)
    ...
    n = tf.compat.v1.where(step > 1, n * f, n)
  ...
  def unbottleneck(self, b, res_size, reuse=None):
    ...
    if self.hparams.mode == tf_estimator.ModeKeys.EVAL:
      return tf.compat.v1.layers.dense(x, res_size, name="dual_unbottleneck_t")
    xt, xi = tf.split(x, 2, axis=0)
    xt = tf.compat.v1.layers.dense(xt, res_size, name="dual_unbottleneck_t")
    xi = tf.compat.v1.layers.dense(xt, res_size, name="dual_unbottleneck_i")
...
class AutoencoderStacked(AutoencoderResidualDiscrete):
  ...
  def stack(self, b, size, bottleneck_bits, name):
    with tf.compat.v1.variable_scope(name + "_stack"):
  ...
  def unstack(self, b, size, bottleneck_bits, name):
    with tf.compat.v1.variable_scope(name + "_unstack"):
      unb = self.unbottleneck(b, size)
      dec = self.decoder(unb)
      pred = tf.compat.v1.layers.dense(dec, bottleneck_bits, name="pred")
      pred_shape = common_layers.shape_list(pred)
      pred1 = tf.reshape(pred, pred_shape[:-1] + [-1, 2])
      x, y = tf.split(pred1, 2, axis=-1)
      x = tf.squeeze(x, axis=[-1])
      y = tf.squeeze(y, axis=[-1])
      gt = 2.0 * tf.cast(tf.less(x, y), dtype=tf.float32) - 1.0
      gtc = tf.tanh(y - x)
  ...
  def stack_loss(self, b, b_pred, name):
    with tf.compat.v1.variable_scope(name):
      labels_discrete = tf.cast((b + 1.0) * 0.5, dtype=tf.int32)
