...
def launch():
  ...
  tf.compat.v1.logging.info("Launched %s. See console to track: %s.", job_name,
                  CONSOLE_URL)
  tf.compat.v1.logging.info("Interact with the training job from the command line:")
  tf.compat.v1.logging.info("Abort job: gcloud ml-engine jobs cancel %s", job_name)
  tf.compat.v1.logging.info("Stream logs: gcloud ml-engine jobs stream-logs %s", job_name)
  tf.compat.v1.logging.info("Open tensorboard: tensorboard --logdir %s", train_dir)
